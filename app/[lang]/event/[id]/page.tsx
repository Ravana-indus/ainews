...
export default async function EventPage({ params, searchParams }: { params: { id: string }, searchParams?: Record<string, string> }) {
  ...
  return (
    <main className="mx-auto max-w-screen-sm p-4">
      <header className="flex items-center justify-between mb-4">
        <Link href="/" className="text-blue-600">{t('back', lang)}</Link>
        <LanguageSwitcher />
      </header>
  <h1 className="text-lg font-semibold">{evt.title}</h1>
  ...
  <div className="mt-2 text-sm text-slate-600">Updated {new Date(evt.updatedAt).toLocaleString()} • <ConfidenceBadge value={evt.confidence} /></div>
  <p className="mt-4 text-slate-800">{evt.summary[lang]}</p>
  <section className="mt-4">
    <h2 className="font-semibold mb-2">Detailed</h2>
    <p className="text-slate-800 whitespace-pre-line">{evt.detail[lang]}</p>
  </section>
      <section className="mt-6">
        <h2 className="font-semibold mb-2">{t('outletsFramed', lang)}</h2>
        <ul>
      ...
      {siteSettings.showBiasRadar && (
        <section className="mt-6">
          <h2 className="font-semibold mb-2">{t('biasRadar', lang)}</h2>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="p-2 bg-slate-50 rounded">
              <div className="font-semibold text-slate-700">Radar</div>
              <BiasRadar values={{
          ...
          <div className="mt-2 text-xs text-slate-600">{t('legendBias', lang)}</div>
        </section>
      )}
      ...
      <section className="mt-6">
        <h2 className="font-semibold mb-2">{t('related', lang)}</h2>
        <div className="flex gap-2 flex-wrap">
          {evt.category && <span className="px-2 py-1 text-xs bg-slate-100 rounded-full">{evt.category}</span>}
        </div>
      </section>
      <section className="mt-6">
        <h2 className="font-semibold mb-2">{t('share', lang)}</h2>
        <ShareButtons />
      </section>
      <section className="mt-6">
        <h2 className="font-semibold mb-2">{t('timeline', lang)}</h2>
        <Timeline steps={[
